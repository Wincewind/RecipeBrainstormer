...
def get_search_tags(search_tags: list):
    active_tags = [
        (tag.cget("text"), tag.context) for tag in search_tags if tag.toggled.get() == 1
    ]
    return active_tags
...
def search():
    ...
    for term in search_var.get().replace(",", "").split(" "):
        if term != "":
            active_inc_tags.append((term,"context"))
    active_inc_tags += active_exc_tags
    print(active_inc_tags)
    found_meals.update(find_meals([tag[0] for tag in active_inc_tags],
                                  [tag[0] for tag in active_exc_tags]))
    for name in found_meals:
        if name not in meals.keys():
            create_meal_item(name, "123")
    search_var.set("")
    print(meals)
...
class MealItem:
    def __init__(
        self, meal_name: str, mealid: int, master: tk.Misc, button_txt="+", command=None
    ):
        if not command:
        ...
        self.label = tk.Label(self.frame, text=meal_name)
        self.label.grid(row=1, column=1)
        self.button = tk.Button(self.frame, text=button_txt, command=command)
    ...
    def select_meal(self):
        self.frame.destroy()
        self.__init__(
            self.meal_name, self.mealid, selected_meals_frame, "-", self.deselect_meal
        )
        if not self.meal_details:
            self.meal_details = find_ingredients(self.meal_name)
        self.create_ing_tags()
        self.selected = True

    def create_ing_tags(self):
        for ingredient in self.meal_details:
            create_tag_button(
                ingredient["name"],
                "ingredient",
                ing_rc,
                {"inclusive": inc_ing_tags_frame, "exclusive": exc_ing_tags_frame},
            )
            ingredient_tags.update(ingredient["name"])
...
inc_category_tags_frame = ttk.Frame(
    inc_tag_tab, borderwidth=1, relief="solid", style="tags.TFrame"
)
inc_category_tags_frame.grid(row=3, column=1)
exc_category_tags_frame = ttk.Frame(
    exc_tag_tab, borderwidth=1, relief="solid", style="tags.TFrame"
)
exc_category_tags_frame.grid(row=3, column=1)

inc_ing_tags_frame = ttk.Frame(
    inc_tag_tab, borderwidth=1, relief="solid", style="tags.TFrame"
)
inc_ing_tags_frame.grid(row=4, column=1, pady=10)
exc_ing_tags_frame = ttk.Frame(
    exc_tag_tab, borderwidth=1, relief="solid", style="tags.TFrame"
)
